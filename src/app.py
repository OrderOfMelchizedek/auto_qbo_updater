...
def upload_files():
    ...
    try:
        ...
        response_data = {"progressSessionId": session_id, "processingStarted": True}
        ...
        if not qbo_authenticated:
            warnings.append(
                "QuickBooks is not connected. Customer matching will be skipped. Please connect to QuickBooks to enable automatic customer matching."
            )

        # Check if we should use concurrent processing
        use_concurrent = len(files) > 1 or any(
            file.filename.lower().endswith(".pdf") for file in files if file.filename
        )
        ...
        if use_concurrent:
            ...
            for file in files:
                ...
                try:
                    ...
                    if file_size > MAX_FILE_SIZE:
                        errors.append(
                            f"File too large: {original_filename} ({file_size / 1024 / 1024:.1f}MB, max {MAX_FILE_SIZE / 1024 / 1024}MB)"
                        )
        ...
        if session["donations"]:
            log_progress(
                f"Successfully processed {final_count} donation records. Ready for QuickBooks!",
                force_summary=True,
            )
            progress_logger.end_session(session_id)
    ...
    return jsonify(
        {
            ...
            "recommendation": (
                ...
                if app.config.get("SESSION_TYPE") in ["redis", "filesystem"]
                else "Consider server-side storage"
            ),
        }
    )
    ...
    return jsonify(
        {
            ...
            "tokenExpiry": (
                ...
                if (
                    hasattr(qbo_service, "token_expires_at")
                    and qbo_service.token_expires_at
                    and qbo_service.token_expires_at > 0
                )
                else None
            ),
    ...
    try:
        ...
        if success:
            ...
            log_audit_event(
                ...
                request_ip=request.remote_addr,
                details={
                    "realm_id": sanitize_for_logging(realmId),
            ...
            try:
                ...
                flash(
                    f"Successfully connected to QuickBooks Online. Retrieved {customer_count} customers.",
                    "success",
                )
            except QBOAPIException as e:
                logger.error(f"Error pre-fetching customers: {str(e)}")
                flash(
                    "Connected to QuickBooks Online, but had trouble retrieving customers.",
                    "warning",
                )
            except Exception as e:
        ...
        if donation["internalId"] == donation_id:
            ...
            if customer_lookup:
                ...
                if customer:
                    ...
                    return jsonify({"success": True, "customerFound": False})
    ...
    for i, donation in enumerate(donations):
        if donation["internalId"] == donation_id:
            ...
            if donation.get("qboSalesReceiptId"):
                print(
                    f"Sales receipt already exists for donation {donation_id} with ID: {donation['qboSalesReceiptId']}"
                )
                return (
                    jsonify(
                        {
            ...
            sales_receipt_data = {
                ...
                "CustomerMemo": {"value": f"auto import on {today}"},
            }
            ...
            if result and "Id" in result:
                ...
                return jsonify({"success": True, "salesReceipt": result})
            else:
                donation["qbSyncStatus"] = "Error"
                donations[i] = donation
                session["donations"] = donations
...
def create_batch_sales_receipts():
    ...
    print(
        f"Sending batch sales receipts with default itemRef: {default_item_ref}, depositToAccountId: {default_account_id}, paymentMethodId: {default_payment_method_id}"
    )
...
def create_item():
    ...
    try:
        ...
        qbo_item_data = {
            ...
            "IncomeAccountRef": {"value": item_data["incomeAccountId"]},
            "Active": True,
        }
...
def debug_donations():
    ...
    debug_info = {"total_count": len(donations), "donations_by_key": {}}
