...
try:
    ...
    from .retry import exponential_backoff, retry_on_failure
except ImportError:
...
class QBOService:
    ...
    def __init__(
        ...
        if environment not in ["sandbox", "production"]:
            raise ValueError(
                f"Environment must be 'sandbox' or 'production', got: {environment}"
            )
    ...
    @retry_on_failure(
        max_attempts=3, exceptions=(requests.RequestException, QBOAPIException)
    )
    def get_tokens(self, authorization_code: str, realm_id: str) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.token_expires_at = int(time.time()) + token_data.get(
                    "expires_in", 3600
                )
                ...
                logger.error(
                    f"Failed to get QBO tokens: {response.status_code} - {response.text}"
                )
                return False
                ...
                if expires_at > 0 and int(time.time()) >= (expires_at - 60):
                    ...
                    if not success:
                        raise QBOAPIException(
                            ...
                            user_message="Authentication expired. Please reconnect to QuickBooks.",
                        )
    ...
    def find_customer(self, customer_lookup: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if data["QueryResponse"].get("Customer"):
                    print(
                        f"Strategy 2 - Partial match found: {data['QueryResponse']['Customer'][0].get('DisplayName')}"
                    )
                    return data["QueryResponse"]["Customer"][0]
            ...
            if len(name_parts) >= 2:
                ...
                if "," not in safe_lookup:  # Only if original doesn't have a comma
                    ...
                    if response.status_code == 200:
                        ...
                        if data["QueryResponse"].get("Customer"):
                            print(
                                f"Strategy 3 - Reversed name match found: {data['QueryResponse']['Customer'][0].get('DisplayName')}"
                            )
                            return data["QueryResponse"]["Customer"][0]
                # Try comma-separated to space-separated conversion
                    ...
                    if len(parts) == 2:
                        ...
                        escaped_space_separated = self._escape_query_value(
                            space_separated
                        )
                        ...
                        if response.status_code == 200:
                            ...
                            if data["QueryResponse"].get("Customer"):
                                print(
                                    f"Strategy 3b - Comma to space conversion match found: {data['QueryResponse']['Customer'][0].get('DisplayName')}"
                                )
                                return data["QueryResponse"]["Customer"][0]
            ...
            if significant_parts:
                ...
                for significant_part in significant_parts:
                    if (
                        len(significant_part) > 3
                    ):  # Only use tokens with more than 3 chars
                        ...
                        if response.status_code == 200:
                            ...
                            if data["QueryResponse"].get("Customer"):
                                print(
                                    f"Strategy 4 - Significant part match found: {data['QueryResponse']['Customer'][0].get('DisplayName')} (matched on '{significant_part}')"
                                )
                                return data["QueryResponse"]["Customer"][0]
                ...
                if len(email_parts) == 2 and "." in email_parts[1]:
                    ...
                    if len(org_name) > 3:  # Only use if meaningful
                        ...
                        if response.status_code == 200:
                            ...
                            if data["QueryResponse"].get("Customer"):
                                print(
                                    f"Strategy 5 - Email domain match found: {data['QueryResponse']['Customer'][0].get('DisplayName')} (matched on '{org_name}')"
                                )
                                return data["QueryResponse"]["Customer"][0]
            ...
            if (
                ...
                if (
                    len(cleaned_phone) >= 7
                ):  # Need at least 7 digits for meaningful phone match
                    ...
                    if response.status_code == 200:
                        ...
                        if data["QueryResponse"].get("Customer"):
                            print(
                                f"Strategy 6 - Phone match found: {data['QueryResponse']['Customer'][0].get('DisplayName')} (matched on phone ending in '{cleaned_phone[-7:]}')"
                            )
                            return data["QueryResponse"]["Customer"][0]
    ...
    def get_customer_cache_stats(self) -> Dict[str, Any]:
        ...
        with self._cache_lock:
            cache_size = len(
                [k for k in self._customer_cache.keys() if not k.startswith("id_")]
            )
            cache_age_seconds = 0
    ...
    def create_customer(
        ...
        try:
            ...
            response = requests.post(
                url, headers=self._get_auth_headers(), json=customer_data
            )
            ...
            if response.status_code == 200:
                ...
                print(
                    f"Error creating customer: {response.status_code} - {response.text}"
                )
                return None
    ...
    def update_customer(
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(
                    f"Error updating customer: {response.status_code} - {response.text}"
                )
                return None
    ...
    def find_sales_receipt(
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(
                    f"Error querying sales receipts: {response.status_code} - {response.text}"
                )
                return None
    ...
    def create_sales_receipt(
        ...
        try:
            ...
            response = requests.post(
                url, headers=self._get_auth_headers(), json=sales_receipt_data
            )
            ...
            if response.status_code == 200:
                ...
                try:
                    ...
                    if "Fault" in error_json:
                        ...
                        error_detail = (
                            error_json["Fault"].get("Error", [{}])[0].get("Detail", "")
                        )
                        error_message = (
                            error_json["Fault"].get("Error", [{}])[0].get("Message", "")
                        )
                        error_code = (
                            error_json["Fault"].get("Error", [{}])[0].get("code", "")
                        )
                        ...
                        if "Invalid Reference Id" in error_message:
                            ...
                            if (
                                ...
                                if account_match:
                                    ...
                                    print(
                                        f"Detected invalid account reference: {account_id}"
                                    )
                            ...
                            # Item reference errors - multiple possible error formats
                                ...
                                item_match = re.search(
                                    r"Item elements id (\d+)", error_detail
                                )
                                if not item_match:
                                    item_match = re.search(
                                        r"Item elements Id (\d+)", error_detail
                        ...
                        # Check for duplicate document number
                            ...
                            "Duplicate" in error_message and "DocNumber" in error_detail
                        ):
    ...
    def get_all_customers(self, use_cache: bool = True) -> List[Dict[str, Any]]:
        ...
        if use_cache and self._is_cache_valid():
            with self._cache_lock:
                ...
                if cached_customers:
                    logger.info(
                        f"Returning {len(cached_customers)} customers from cache"
                    )
                    return cached_customers

        if not self.access_token or not self.realm_id:
            logger.error(
                "Not authenticated with QBO - Missing access_token or realm_id"
            )
        ...
        try:
            ...
            while True:
                ...
                print(
                    f"Batch {batch_count}: Requesting customers at position {start_position}"
                )
                ...
                if response.status_code == 200:
                    ...
                    if batch and len(batch) > 0:
                        ...
                        print(
                            f"Sample customer - ID: {first_customer.get('Id')}, Name: {first_customer.get('DisplayName')}"
                        )
                    ...
                    if len(batch) < max_results:
                        print(
                            f"Batch {batch_count}: Less than max results, finished retrieving"
                        )
                        break
            ...
            raise QBOAPIException(
                f"Network error retrieving customers: {str(e)}",
                user_message="Unable to connect to QuickBooks. Please check your internet connection.",
            )
        except Exception as e:
            logger.error(f"Unexpected error in get_all_customers: {str(e)}")
            raise QBOAPIException(
                f"Unexpected error retrieving customers: {str(e)}",
                user_message="An unexpected error occurred while retrieving customers.",
            )
    ...
    def create_account(self, account_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            response = requests.post(
                url, headers=self._get_auth_headers(), json=account_data
            )
            ...
            if response.status_code == 200:
                ...
                print(
                    f"Error creating account: {response.status_code} - {response.text}"
                )
                return None
    ...
    def create_item(self, item_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            response = requests.post(
                url, headers=self._get_auth_headers(), json=item_data
            )

            if response.status_code == 200:
                item = response.json().get("Item")
                print(
                    f"Successfully created item: {item.get('Name')} (ID: {item.get('Id')})"
                )
                return item
            else:
    ...
    def create_payment_method(
        ...
        try:
            ...
            response = requests.post(
                url, headers=self._get_auth_headers(), json=payment_method_data
            )
            ...
            if response.status_code == 200:
                ...
                print(
                    f"Error creating payment method: {response.status_code} - {response.text}"
                )
                return None
    ...
    def get_all_items(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            while True:
                ...
                print(
                    f"Batch {batch_count}: Requesting items at position {start_position}"
                )
    ...
    def get_all_accounts(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            while True:
                ...
                print(
                    f"Batch {batch_count}: Requesting accounts at position {start_position}"
                )
            ...
            print(
                f"Successfully retrieved {len(accounts)} accounts in {batch_count} batches"
            )
