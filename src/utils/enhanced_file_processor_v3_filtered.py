"""
Enhanced file processor V3 with filtering of payments without payer info.
"""
...
class EnhancedFileProcessorV3:
    ...
    def process_files(self, files: List[Tuple[str, str]]) -> Tuple[List[Dict[str, Any]], List[str]]:
        ...
        # Step 6: Filter out payments without valid payer info
        ...
        for payment in enriched_payments:
            ...
            if has_customer_lookup or has_org_name or has_full_name:
                ...
                logger.warning(f"Discarding payment without payer info: Check/Ref {check_ref}, Amount ${amount}")

        discarded_count = initial_count - len(valid_payments)
        if discarded_count > 0:
            logger.info(f"Discarded {discarded_count} payments without valid payer information")
        ...
        logger.info(f"Processing complete: {len(valid_payments)} valid payments")
        ...
        logger.info(f"  - Discarded (no payer): {discarded_count}")
