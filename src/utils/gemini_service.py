...
class GeminiService:
    ...
    def match_donation_with_customers(self, donation: Dict[str, Any], qbo_customers: List[Dict[str, Any]]) -> Dict[str, Any]:
        """Match a donation with QuickBooks customer records using Gemini.
        
        Args:
            donation: Extracted donation data
            qbo_customers: List of all QuickBooks customer records
            
        Returns:
            Match result dictionary containing match status, confidence, and updated donation
        """
        try:
            print("==== STARTING AI-BASED DONATION-CUSTOMER MATCHING ====")
            print(f"Donation info: Name: '{donation.get('Donor Name', 'Unknown')}', Address: '{donation.get('Address - Line 1', 'Unknown')}'")
            print(f"Total customers to match against: {len(qbo_customers)}")
            
            # Set up model
            model = genai.GenerativeModel('gemini-2.5-pro-preview-03-25')
            
            # Get the donor matching prompt
            matching_prompt = self.prompt_manager.get_prompt('fom_contact_list_assistant')
            if not matching_prompt:
                print("ERROR: Failed to get matching prompt from prompt manager")
                raise ValueError("Missing donor matching prompt")
                
            print(f"Matching prompt loaded: {len(matching_prompt)} characters")
            
            # Limit the number of customers to avoid exceeding context limits
            max_customers = 500  # Adjust based on model limits and testing
...
Analyze the donation details and find the best matching customer from the QuickBooks customer list.
Return the match result in the JSON format specified in the instructions.
"""
            prompt_length = len(prompt_with_data)
            print(f"Complete prompt size: {prompt_length} characters")
            
            print(f"Making Gemini API call for matching...")
            # Call Gemini API with timing info
            import time
            ...
            print(f"Gemini API call completed in {elapsed_time:.2f} seconds")
            
            # Extract and parse the response
            text_response = response.text
            if text_response:
                print(f"Response received: {len(text_response)} characters")
                print(f"Response preview: {text_response[:200]}...")
                
                match_result = self._extract_json_from_text(text_response)
                if match_result:
                    print("==== MATCHING RESULT ====")
                    print(f"Match found: {match_result.get('matched', False)}")
                    print(f"Confidence: {match_result.get('matchConfidence', 0)}")
                    
                    if match_result.get('matched', False):
                        customer_match = match_result.get('customerMatch', {})
                        print(f"Matched to: {customer_match.get('DisplayName', 'Unknown')}")
                        print(f"Customer ID: {match_result.get('customerMatchId', 'Unknown')}")
                        print(f"Address changed: {match_result.get('addressChanged', False)}")
                    
                    print(f"Needs review: {match_result.get('needsReview', True)}")
                    if match_result.get('needsReview', True):
                        print(f"Review reason: {match_result.get('reviewReason', 'Not specified')}")
            ...
            # Default response if matching fails
            print("Failed to match donation with customers - returning default response")
            return {
                "matched": False,
                "matchConfidence": 0,
                "updatedDonation": donation,
                "needsReview": True,
                "reviewReason": "Matching process failed",
                "matchingNotes": "The AI matching process did not return a valid result."
            }
            
        except Exception as e:
            print(f"ERROR in match_donation_with_customers: {str(e)}")
            import traceback
            traceback.print_exc()
            ...
            return {
                ...
                "updatedDonation": donation,
                "needsReview": True,
                "reviewReason": "Error during matching process",
                "matchingNotes": f"An error occurred during matching: {str(e)}"
            }
