...
class TestDonationValidator:
    ...
    def test_clean_check_number(self, validator):
        ...
        assert (
            validator.clean_check_number("0012345") == "12345"
        )  # 7 digits, leading zeros
        assert (
            validator.clean_check_number("123456") == "123456"
        )  # 6 digits, no leading zeros
        assert validator.clean_check_number("000000") == "0"  # 6 digits, all zeros
        assert (
            validator.clean_check_number("000001") == "1"
        )  # 6 digits, leading zeros to single digit
        ...
        assert (
            validator.clean_check_number("1234") == "1234"
        )  # 4 digits, no leading zero
        assert (
            validator.clean_check_number("00123") == "00123"
        )  # 5 digits, leading zeros
        assert (
            validator.clean_check_number("12345") == "12345"
        )  # 5 digits, no leading zeros
        ...
        assert (
            validator.clean_check_number("ABCDEF") == "ABCDEF"
        )  # Non-numeric, 6 chars
        assert (
            validator.clean_check_number("00ABCD") == "00ABCD"
        )  # Mixed, 6 chars, leading numeric looking
