...
class DeduplicationService:
    ...
    @staticmethod
    def deduplicate_donations(existing_donations: List[Dict[str, Any]], 
                            new_donations: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        ...
        logger.info(f"Deduplication complete: {len(result)} unique donations "
                   f"(merged {merge_count}, added {new_count})")
        
        # Log final customer match status
        matched_count = sum(1 for d in result 
                          if d.get('qbCustomerStatus') in ['Matched', 'Matched-AddressMismatch', 
    ...
    def merge_donation_data(existing: Dict[str, Any], new: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Debug logging
        logger.info(f"Merging donations - Existing status: {existing.get('qbCustomerStatus')}, "
        ...
        logger.info(f"Merge complete - Result status: {merged.get('qbCustomerStatus')} - "
                   f"ID: {merged.get('qboCustomerId')}")
        
        return merged
    
    @staticmethod
    def _merge_customer_fields(merged: Dict[str, Any], existing: Dict[str, Any], 
        ...
        existing_matched = existing.get('qbCustomerStatus') in [
            'Matched', 'Matched-AddressMismatch', 'Matched-AddressNeedsReview'
        ]
